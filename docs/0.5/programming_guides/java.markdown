...
First, you obtain an `ExecutionEnvironment`, then you specify where data should
be loaded from, then you specify transformations on this data, then you
tell the system where to store the results of your computations. The last step
is then telling the system to execute your program on a cluster or on your
local computer.

We well now give an overview of each of those steps but for more details
on how to load data, apply transformations, and store data please refer
...
will to the right thing depending on the context: if you are executing
your program inside an IDE or as a regular Java program it will create
a local environment that will execute your program on your local machine. If
you created a jar file from you program the Stratosphere cluster manager will
execute you main method and `getExecutionEnvironment()` will return
an execution environment for executing your program on a cluster.

For specifying date sources the execution environment has several methods
...
Once you have a `DataSet` you can apply transformations to create new
a new `DataSet` which you can then write to a file, transform again, or
